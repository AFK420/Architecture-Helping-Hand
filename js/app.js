 ...
 */
const STAIR_REFERENCE_DEFAULTS = Object.freeze({
  riser: Object.freeze({
...
const STAIR_INPUT_MODES = Object.freeze({
...
const STAIR_ERROR_CODES = Object.freeze({
 ...
 */
function resolveStairReferences(overrides = {}) {
  const merged = {};
 ...
 */
function evaluateRangeStatus(valueMeters, range) {
 ...
 */
function formatStairResult(result, displayUnit = 'mm', precision = 0) {
  const f = {
 ...
 * @returns {string} SVG markup
 */
function generateStairSVG(result, options = {}) {
...
const PROJECT_LIBRARY_KEY = 'archiscale_project_library';
 ...
 */
function touchProject(doc, when) {
 ...
 */
function placeFurniture({ id, name, catalogId, displayName, wCm, dCm, x, y, rotated = false, roomId = null }) {
 ...
 */
function createViewTransform(init = {}) {
  return {
...
function zoomAt(transform, factor, svgX, svgY, minZoom = 4, maxZoom = 400) {
 ...
 */
function buildGrid(transform, svgWidth, svgHeight, gridMeters = 0.5, majorEvery = 4, maxLines = 200) {
...
function entityAddRemoveCommand(list, entity, label) {
  let removedIndex = -1;
  return {
    label,
    redo() {
 ...
 */
function generatePlanSVG(geometry, options = {}) {
...
const VISUAL_CAPABILITIES = Object.freeze(['vision', 'imageGen']);

/** What the visual layer can do with a given provider. */
function visualCapabilities(provider) {
  return {
 ...
 */
async function interpretImage(provider, { imageBase64, mimeType, question }) {
 ...
 */
async function generateConceptImage(provider, { prompt }) {
  const caps = visualCapabilities(provider);
...
function createRescalerView(context) {
...
function createDetectorView(context) {
  const { state, dom, setUnifiedResultState } = context;
...
function createAreaVolumeView(context) {
  const { state, dom, setUnifiedResultState } = context;
...
function createChainsView(context) {
  ...
  // =========================================================================
  // MODULE: ViewCadClipboardHandoff
  // =========================================================================
...
function createCadClipboardView(context) {
...
function createCadHandoffView(context) {
  const { state, dom, showToast, copyToClipboard, setUnifiedResultState, AudioService, StorageService, switchMode } = context;
...
function createBatchCadView(context) {
  const {
    state, dom, showToast, copyToClipboard, downloadFile, setUnifiedResultState,
...
function createQuickDimensionView(context) {
  const {
...
function createHistoryView(context) {
  const {
...
function createProjectsView(context) {
  const {
    state, dom, showToast, setUnifiedResultState, AudioService,
