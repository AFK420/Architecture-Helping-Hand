...
* **Unique Top-Level Name Rule (added after the stair-corruption incident)**: the bundle concatenates every module into ONE shared function scope — two modules declaring the same top-level `function name(...)` (or `const`) collide, and JavaScript hoisting makes the LAST declaration win for the entire bundle. This silently corrupted stair geometry (ramp's `buildGeometry(rise, run)` hoisted over the stair's 4-argument version: angle 9.9° instead of 33.7°, run 16 m instead of 4.2 m, `NaN` path attributes in the stair SVG). Never declare generic top-level helper names (`fmt`, `buildGeometry`, `requireLengthMeters`, …); prefix them per module (`stairFmt`, `buildRampGeometry`, …). Enforced by `tests/build-integrity.test.js`, which parses the generated bundle and fails if any top-level function name is declared more than once.
